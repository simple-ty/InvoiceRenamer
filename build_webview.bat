@echo off
chcp 65001 >nul 2>&1
title InvoiceRenamer WebView 版打包脚本

echo ============================================
echo   InvoiceRenamer WebView 版打包脚本
echo ============================================
echo.
...
echo [2/3] 开始打包 WebView 版（无 UPX 压缩）...
"%VENV_PY%" -m PyInstaller webapp.spec --noconfirm
if errorlevel 1 (
    echo.
    echo [错误] 打包失败！
    pause
...
echo [3/3] 打包结果:
set "FOUND_EXE="
for %%f in ("dist\InvoiceRenamer_WebView_*.exe") do set "FOUND_EXE=%%f"
if defined FOUND_EXE (
    ...
    echo [错误] 未找到输出文件 dist\InvoiceRenamer_WebView_*.exe
)
echo.
