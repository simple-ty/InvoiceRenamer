...
- **自动检查更新**：启动时静默检查 + Header 手动按钮，双重更新源（jsDelivr CDN / GitHub API），国内可用
...
| 版本 | 主要更新 |
|---|---|
| **v0.5.18** | 自动检查更新（启动静默检查 + Header 手动按钮）；双重更新源（jsDelivr CDN 国内优先 + GitHub API 兜底）；24h 本地缓存，零新增依赖 |
| **v0.5.17** | 版本更新检查基础实现：GitHub Releases API 检查，有新版时 Header 下方显示内联提示条 |
| **v0.5.16** | 设置对话框修复：密钥验证兜底、眼按钮 flex 布局修复点击失效、提示文案精简；清理测试文件 |
